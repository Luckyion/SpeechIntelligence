...
/**
 * 捕获全局异常，当Application出现未能捕获的异常</br> 或者其他异常造成程序出现“应用程序已停止运行”的错误时</br> 采用自定义
 * {@link Application}(如MyApplication)继承Application</br> 并在
 * {@link AndroidManifest.xml}中将<application></application>标签修改</br>
 * 自定义的Application。应用程序在出现不可控异常时将调用自定义Application</br>
 * 在其中做相应的异常捕获，并做后续处理，如获取设备信息，获取异常信息</br> 并友好的提示用户出现了不可控的异常，导致应用程序退出。 </br>
 * 
 * @author Vickie Tang
 * @since version 1.0
 * 
	...
	private void showDialog() {
		new Thread() {
			...
			public void run() {
				...
				.setCancelable(false)
				.setMessage("对不起，俺奔溃啦 - -")
				.setNeutralButton("原谅你啦!!!", new OnClickListener() {
					@Override
